...
const { runtime, glob } = getGlobalContext();

let routes: any;
const pattern = /\[\s*(.*?)\s*\]/;

export async function generateRoutes() {
  const files = await glob("**/*.yapapa", { cwd: "./src/pages" });

  const routes = [];

  for await (const file of files) {
    const name = file.replace("index.yapapa", "");
...
export async function loadRoutes() {
  const file = await runtime.readFile("./.yapapa/routes.manifest.json");
...
export async function getRouteFile(pathname: string) {
  ...
  return {
    path: "./src/pages/404/index.yapapa",
    params: {},
  };
}
...
export function getNormalRoute(pathname: string) {
  const route = routes.find(
    (route: any) => !route.dynamic && route.name === pathname,
  );
